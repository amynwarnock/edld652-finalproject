...
#I'm not sure if you're also getting these errors but it looks like there are some parsing errors created in the next steps
...
## Super interesting and informative plot! It might help to have the axis labels tiled a bit to aid readability. My eye is drawn to the difference in the homeless group and I wonder how much of those results are impacted by the (presumably) lower sample size. It could be helpful to give the median sample size for each plot to get a bit more grounding (but I do hesitate to suggest adding more information since this is already so dense)
...
## I think this is my vote for best plot so far - it manages to convery a lot of the information the others do but with much more simplicity. I really got the picture by looking at this. It could be even simpler if you aggregate over the entire country
...
# Beautiful plot, really dig the aesthetics of this one. However, I'm not sure if I follow what's going on here based on the title - particularly the "by gender" portion. My understanding based on the code is that this is just all of the data before being separated into economic/other subgroups? I might not be fully comprehending this but it seems to me the title could just be "Mean percentage of students that scored at or above proficient" 
```
...
## Very cool additional representations of the data! For what it's worth, I think the ridgeline plots convey this information the most succinctly and clearly. If you want to capture all of these different groups I might suggest a series of those plots facet-gridded like these geographic plots so you can have all of the subgroups together
```
...
## I think the column plots do a great job of showing the overall instruction spending. In my opinion, the second to last one is probably the best - although the annotation is cool, the line showing the mean is kind of hard to follow so I'm leaning towards the simplicity of the other. 

## As for the last group of plots, I think it's a really cool use of the colors that they change along two dimensions! The only thing I'd say to improve these is to change their size or the font size so it all fits on the plot
```
...
## Really great way of visualizing this! This is my pick for the best way to visualize this data
```
...
# The faceted graphs are a real improvement over the main scatterplot of all the data. I'd be curious to see this with linear models fit (as I edited above, but I feel like this could be cleaned up a bit)
```
