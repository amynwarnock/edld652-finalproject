...
#MV PEER REVIEW: Add a calculation for standard error so that you can create error bars; added one line as an example
dat <- df2 %>% 
	group_by(STNAM) %>% 
	summarise(
		mean_EconomicallyDisadvantaged_math = mean(ECDmath, na.rm = TRUE),
		se_EconomicallyDisadvantaged_math = sd(ECDmath, na.rm = TRUE)/sqrt(n()),
...
#MV PEER REVIEW: I like this graph
df_pivot %>%
