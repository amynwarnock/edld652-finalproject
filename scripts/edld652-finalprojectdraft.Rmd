...
big_states <- c("California","Florida","Texas","Pennsylvania","Ohio","Illinois","North Carolina","Michigan","Arizona","Georgia")


df_pivot%>% filter(STNAM.y %in% big_states) %>%
	...
	theme_minimal()
#Great colors
#I'm worried these y-axis labels will run together at any size, maybe subset some states?
...
#This is a great visualization that I wouldn't have thought of, with a cool idea for the y-axis too!
#I would make sure to pick an aspect ratio that won't cause the y-axis labels to overlap.
#I added a bit of code to subset the most populated 10 states in the first plot.
#My only real critique of these is that there are lots of y-axis labels that may not fit on a reasonably sized graphic.
#Plot 3 is very cool, and has a great amount of information and white space. The regions don't add too much for me and their distributions are so similar that you might take them out. You might still be able to add line segments indicating mean by region.
```
...
dat3 <- dat2 %>% filter(test == "math")%>% group_by(Subgroup) %>% summarize(mean_pct = mean(mean_pct))
levels(dat2$Subgroup) <- dat3[order(dat3$mean_pct),][[1]]
...
#These plots are great too! The scale colors look good, the text is legible for me
#I releveled subgroup to order it based on mean math score, rather than be alphabetical
```
...
#Both of these are very clean. Mapping region to color on the first was a great choice.
#A heatmap of spending by state might also be informative, especially as a colorful supplement to the second chart
```
...
instruction2 %>% group_by(Region) %>% arrange(desc(-E13_B), .by_group = TRUE) %>% mutate(STNAME=factor(STNAME, levels=STNAME)) %>% 
  ggplot(aes(E13_B, STNAME)) +
  geom_col(aes(fill = Region),
           alpha = 0.9) +
  scale_fill_OkabeIto() + 
  scale_x_continuous(expand = c(0, 0), 
...
#These look good, I wanted to see them grouped by region then spending, so I did. Could also arrange regions by mean spending too. It doesn't look as good as the way you originally had it.
```
...
#Great idea! I feel like the only missing is more range between the majority of states. Maybe restrict the overall y range, so that there's more variation between states, at the expense of not being able to tell the difference between the states on either extreme.
```
...
#This is a really pleasing graph. It's also a really cool idea, but it's hard for me personally to interpret and be able to give meaning to the colors. Maybe you could add topography as a dimenion. The font fits really nicely in most of the states too (not sure how you can resolve the northeast without the usual arrows).
```
...
SpEd2  %>%
  ...
  theme(plot.title.position = "plot",
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank()) 
#I like this, maybe think about indicating the mean as a line? Otherwise, I think you've found a very clear compelling format, and I can really only nitpick at this point
...
viz3_rla00long_fiscal_2010 %>% filter(stnam %in% big_states) %>% 
  filter(subgroup == "all")  %>% 
...
#Really cool to see the different distributions. Have you thought about doing a subset of states? I added that above, if you want it again
```
...
#Awesome! The only thing I would add is a regression line with equation and R^2.
```
...
#I think this is a really cool way to show off the distributions. You could convey the relationships on a spaghetti plot, which would be cool too.
```
...
#These still look cool. I think these also would look good as some sort of density chart, so I changed the code to do that. I don't think it's better.
```
...
#You guys keep anticipating my comments! This is great, but I would pick out a few states
```
...
#This is cool, but I feel like it's better served by the version above. I tried combining them and messing with the alpha/size. I would probably also restrict the scale on both sides
```
...
viz3_rla00long_fiscal_2010 %>% filter(stnam %in% big_states)%>% 
  filter(subgroup %in% c("cwd","ecd","lep","mam")) %>% 
  ggplot(aes(x = locrev_stu, y = meanpctprof, color = subgroup)) +
  facet_wrap(~stnam) +
  geom_point(alpha = .4) + 
  scale_x_log10(labels = scales::dollar)

#I continue to like this style, but I think this needs both a subset of states and subgroups to be more legible, so I did that
```
...
#Nice charts, I think the log scale combined with the facet wrapping limits interpreting these, unless the goal was to convey general counts, which this does nicely.
```
