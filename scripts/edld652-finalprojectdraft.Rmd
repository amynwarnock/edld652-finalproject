...
# Even though it is your first mock up, I actually think this visual tells a great story in such a simple way. You can kind of tell how most subgroups show similar trends by state, but that the homeless student population differ from the rest of the cohorts. Could be something to highlight or explore further.
df_pivot %>%
...
# Again, I know this is just an initial mock up, but this one is a lot to look at. Also, the colors may be difficult with how many plots there are. I am not colorblind, but I feel that this visual would be tough(er).
df_pivot %>%   
	ggplot(aes(x = mean_pct, 
...
# I think this an awesome thing to work off of. It would be great if you could expand the row ratio of the plot or find a better alpha level to ensure the viewer can read correctly. 
#Also, unsure about what mean percent means when the x axis is 0-100. What is the definition of proficient in this case? For what test? Math and reading? Make this clear in the title/subtitle. 
...
# Regardless of having it split to male/female. I like this color pallete for the density plot. It seems to pop more.
df_pivot %>%
...
#Math only
# I realy like this. I might would suggest making it a bit more clear that this is math scores only. The subtitle is kind of hidden on the left margin of the plot. Consider reformatting title and subtitle location.
dat2 %>% 
	filter(test == "math") %>% 
...
ggplot(data = dat2, aes(geometry = geometry, 
	...
	labs(title = "Mean percentage of students that scored at or above proficient",
			 ...
			 caption = "Source: U.S. Department of Education") +
	theme_void()
# This is awesome and tells the whole picture! Awesome! 
```
...
# All of this data wrangling is incredibly impressive. I wish that I had a better grasp on data wrangling so I could give more feedback. After looking at the completed data frames, it all seems to have come out as intended. Again, very well done and I am jealous of your abilities! :)
...
# Great start with the first plot. I like how you all have standardized title fonts already!
...
^^ Yeah, I am not sure if this is really needed. If you want something like this, maybe consider changeing the type of plot. If there is a way that you could transform this into a density plot of some sort, that may tell the story a bit better?
...
total2  %>%
  ...
  geom_text(
    ...
    nudge_x = 0.45,
    size = 2) +
  geom_vline(aes(xintercept = mean(total2$spend_per_stu_k)), # I really like the use of the v-line for the National Average. Another way to do this could be to had a National Average row to your DF so that you could have it on the Y-axis. You could then highlight it a different color. You could also highlight all those below and all of those above with a different bar border color. 
             data = total2, 
             linetype = 'dashed',
             size = 0.6,
             alpha = 1, 
...
total_rg_avg_stu  %>%
  ...
  theme(plot.title.position = "plot",
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank()) 
# Again, the region plot. . .  I do not think it helps progress the story at all. The prior plot is great though. If you really want to show regions, maybe you could add them as rows to your state column so you could plot them next to all the states. Could help show the full picture.
```
...
# You all have been so great with the maps. Like so awesome. I really like the color scale you use for this one.
```
...
instruction2  %>%
  ...
  theme(plot.title.position = "plot",
        panel.grid.major.y = element_blank(),
        panel.grid.minor.y = element_blank()) 
# again, great. One suggestion would be to have an additional caption that briefly describes what "Spending on Instruction" is. 
```
...
instruction2  %>%
  ...
  labs(title = "Total Spending on Instruction Per Single Student by State",
       ...
       fill = "Region") + 
  geom_text(
    aes(instr_per_stu_k, STNAME, label = paste (round(instr_per_stu_k, 2), " K")), show.legend = FALSE, # I tried to add show.legend = FALSE and it would not work. this is all I could find on stack overflow. I am sure Daniel would have a quick fix for this.
    nudge_x = 0.45,
    size = 2) +
  ...
  geom_label(data = instruction2, 
             aes(x = mean(instr_per_stu_k)+1.1, 
                 y = 10,
                 label = "Avg. Spending
on Istruction
Per Student 
...
# THIS. IS. AMAZING. Though it takes a little bit of work to read the legend at first, It is easy to understand after. This is beautiful.

# One suggestion, use geom_text() (i think) to make it to where the labels for the smaller states are off tho the side of the map with lines connecting them to the correct state. This will make the NE a little less cluttered.
```
...
# Is there a reason some of these seem to go off on the y axis. Do some localities really not get local revenue from property taxes??
```
...
viz3_rla00long_fiscal_2010 %>% 
  ...
  theme(plot.title.position = "plot",
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())
# Consider assigning color to each state? Add a trend line?
```
...
viz3_rla00long_fiscal_2010 %>% 
  ...
  theme(plot.title.position = "plot",
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank()) 
# I am ignorant as to how, but could you do what Daniel did at the beginning of the term and bin the data so that the amount of points would be limited but still show the whole picture?
...
viz3_rla00long_fiscal_2010 %>% 
  ...
  ggplot(aes(x = locrevtaxes_stu, y = meanpctprof)) +
  facet_wrap(~stnam) +
  geom_jitter(color = "gray30", fill = "gray30", alpha = .4) + #tried to fix with geom_jitter to no avail.
  scale_x_log10(labels = scales::dollar) +
  labs(title = "Local Revenue from Property Taxes and RLA Proficiency",
       y = "Approximate Average Percent Proficient",
       x = "Dollar per Student",
...
viz3_rla00long_fiscal_2010 %>% 
  ...
  theme(plot.title.position = "plot",
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())
# You should relabel your cohorts so that they are more descriptive rather than abbreviations I believe you can use recode_factor() or could try levels().
levels(viz3_rla00long_fiscal_2010$subgroup) <- list(All = "all", AmericanIndianorAlaskanNative = "mam", 
AsianPacificIslander = "mas",
Hispanic = "mhi",
Black = "mbl",
White = "mwh",
TwoOrMoreRaces = "mtr",
ChildrenWithDisabilities = "cwd",
EconomicallyDisadvantaged = "ecd",
LimitedEnglishProficiency = "lep") # Tried this but it did not work :(
...
viz3_rla00long_fiscal_2010 %>% 
  ...
  theme(plot.title.position = "plot",
        panel.grid.minor.y = element_blank(),
        panel.grid.minor.x = element_blank())
# Again, consider binning to limit the amount of points present.
```
